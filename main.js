...
var stage = new Container(), renderer = autoDetectRenderer(256, 256);
...
function insertChecker() {
    ...
    for(let y = maxCol-1; y > -1; y--) {
        if(boardArray[x][y].sprite.visible == false){
            countCheckers++;//Keep track of the amount of placed checkers
            boardArray[x][y].sprite.visible = true;
            //Place the checker with the right color
            if(turns == 0){
...
function checkForWin (x,y,color) {   
    ...
    if (countCheckers >= 42) {
        ...
        checkSlotsDiagonallyCaseOne(x, y, color);              
        checkSlotsDiagonallyCaseTwo(x, y, color);        
    }
    if (gameOver) {
...
function checkSlotsBelow(x,y,color) {    
    ...
    for (let i = y + 1; i < maxCol; i++) {
        if ((boardArray[x][i].sprite.visible == true) && (boardArray[x][i].color == color)) {
            ...
            return; // if there is no matching checker below you cannot win
        } 
    }
};
...
function checkSlotsDiagonallyCaseOne(x,y,color) {
    //Up and right
    var score = 1;
    var j = y-1;
...
function checkSlotsDiagonallyCaseTwo(x,y,color)
{
    //Up and left
    var score = 1;
    var j = y-1;
    ...
    for(let i = x+1; i < maxRow; i++) {    
        ...
        if((boardArray[i][m].sprite.visible == true) && (boardArray[i][m].color == color)){
            ...
            if(score == 4){
                ...
                break;//There is no matching checker so we can't win
        }
